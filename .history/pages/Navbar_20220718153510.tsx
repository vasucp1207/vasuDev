...
function Navbar() {
  ...
  return (
    <div>
      <div className='absolute top-0 bg-white rounded-2xl text-black'>
      ...
      {!active && <div className='z-10 h-20 w-screen fixed z-40 bg-transparent flex justify-center items-center'>
        <div className='flex gap-8 text-xl font-normal mr-5 items-center justify-center'>
          ...
          {!user && <button onClick={signInWithGoogle} className='z-30 hover:outline-none transition-all duration-300 bg-transparent
            hover:bg-gradient-to-r from-violet-500 to-fuchsia-500 w-32 h-12 
            outline rounded-full text-xl'>SignIn</button>}
            {user && <button onClick={signOutWithGoogle} className='z-30 hover:outline-none transition-all duration-300 bg-transparent
            hover:bg-gradient-to-r from-violet-500 to-fuchsia-500 w-32 h-12 
            outline rounded-full text-xl'>LogOut</button>}
        </div>
      </div>}
      ...
      {active && <div className='backdrop-blur-lg z-10 h-20 w-screen fixed z-40 bg-transparent flex justify-center items-center'>
        <div className='flex space-x-9 text-xl font-normal mr-5 items-center'>
          ...
          {!user && <p onClick={signInWithGoogle} className='hover:text-fuchsia-500 cursor-pointer'>SignIn</p>}
          {user && <p onClick={signOutWithGoogle} className='hover:text-fuchsia-500 cursor-pointer'>LogOut</p>}
        </div>
      </div>}
    </div>
  )
